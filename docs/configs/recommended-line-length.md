...
provides all plugin rules with preset options: sorting by string length in
descending order.
...
Use the `recommended-line-length` configuration when you want your code to look
neat and visually pleasing.

This configuration is especially useful for projects where the aesthetic
appearance of the code is important and where developers appreciate clean
alignment and visual harmony. It helps create code that is easier to read and
perceive by minimizing visual clutter and enhancing the overall structure of the
code.
...
export default [dependencies.configs['recommended-line-length']]
```
...
// .eslintrc.js
module.exports = {
  extends: ['plugin:dependencies/recommended-line-length-legacy'],
}
```
